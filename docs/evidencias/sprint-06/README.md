...
La suite del sprint cubre 13 casos: seis de `DuenoServiceTest`, dos de `TurnoServiceTest` y cinco de `DuenoControllerTest`. El proyecto usa `spring-boot-starter-test` para JUnit y Mockito, y `spring-boot-starter-webmvc-test` para MockMvc.

| Verificación | Resultado |
| --- | --- |
| `.\mvnw.cmd clean test` | `BUILD SUCCESS`; 13 pruebas, 0 fallos, 0 errores, 0 omitidas. La ejecución limpia compiló tres clases de prueba y no inició JPA ni H2. |
| `.\mvnw.cmd test "-Dsurefire.runOrder=random"` | `BUILD SUCCESS`; 13 pruebas, 0 fallos, 0 errores, 0 omitidas. |
